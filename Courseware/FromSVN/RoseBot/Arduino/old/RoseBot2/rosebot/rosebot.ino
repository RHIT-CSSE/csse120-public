...
#define number_of_input_pin_devices 6
...
//      write_byte((byte) right_encoder_count);
      if (command_pin == pinmap[L_ENCODER]) {
        write_two_bytes(encoder.getTicks(LEFT));
      } else if (command_pin == pinmap[R_ENCODER]) {
        write_two_bytes(encoder.getTicks(RIGHT));
      } else {
        write_two_bytes(analogRead(command_pin));
      }
      break;
    case ANALOG_WRITE:
      ...
      break;
    case DIGITAL_READ:
      write_byte((byte) digitalRead(command_pin));
      break;
    case DIGITAL_WRITE:
    	if (command_pin == pinmap[L_ENCODER]) {
