...
class RoseBot(object):
    ...
    def connect_wired(self, port):
        self.connector.connect_wired(port)

    def connect_wireless(self, wifly_number):
        self.connector.connect_wireless(wifly_number)

    def disconnect(self):
        self.connector.disconnect()

    def start_moving(self, left_wheel_pwm, right_wheel_pwm):
        self.motor_controller.drive_pwm(left_wheel_pwm,
                                        right_wheel_pwm)

    def stop_moving(self):
        self.motor_controller.stop()

    def start_left_wheel_moving(self, left_wheel_pwm):
        self.motor_controller.left_wheel_pwm(left_wheel_pwm)

    def stop_left_wheel_moving(self):
        self.motor_controller.left_wheel_pwm(0)

    def start_right_wheel_moving(self, right_wheel_pwm):
        self.motor_controller.left_wheel_pwm(right_wheel_pwm)

    def stop_right_wheel_moving(self):
        self.motor_controller.right_wheel_pwm(0)

    def make_a_noise(self):
        self.buzzer.play_tone(random.randrange(10, 80))
        time.sleep(0.50)
        self.buzzer.stop()
    ...
    def get_left_line_sensor_reading(self):
        return self.sensor_reader.left_reflectance_sensor.read()

    def get_middle_line_sensor_reading(self):
        return self.sensor_reader.middle_reflectance_sensor.read()

    def get_right_line_sensor_reading(self):
        return self.sensor_reader.right_reflectance_sensor.read()

    def get_left_side_proximity_sensor_reading(self):
        return self.sensor_reader.left_proximity_sensor.read()

    def get_front_proximity_sensor_reading(self):
        return self.sensor_reader.front_proximity_sensor.read()

    def get_right_side_proximity_sensor_reading(self):
        return self.sensor_reader.right_proximity_sensor.read()

    def get_left_bump_sensor_reading(self):
        return self.sensor_reader.left_bump_sensor.read()

    def get_right_bump_sensor_reading(self):
        return self.sensor_reader.right_bump_sensor.read()

    def get_left_wheel_encoder_reading(self):
        print('Not yet implemented')

    def get_right_wheel_encoder_reading(self):
        print('Not yet implemented')
...
class Connector(object):
    ...
    def connect(self, port=None, robot_number=None):
        ...
        elif self.simulated_robot:
            self.connect_simulated()
        else:
            print('Connection FAILED:')
            print('You must specify either a PORT\n'
                  + 'or a ROBOT-NUMBER, or have a SIMULATED ROBOT.')
            return
...
#         print('Connected (wired) to port: {}.'.format(self.port))
    ...
    def connect_wireless(self, wifly_number):
        ...
        if isinstance(wifly_number, int):
            self.address = 'r' + str(wifly_number) + suffix
        else:
            print('The wifly number must be an integer.')
            print('Its current (wrong) value is:', wifly_number)
            return
#         else:
#             try:
...
#             self.address = robot_address
    ...
    def connect_simulated(self):
        self._communicator = rosebot.communicator.SimCommunicator(self.sleeper)
        print('Connected to simulated robot:\n{}'.format(
            self.simulated_robot))
        self.on_successful_connection()

    def on_successful_connection(self):
        self.is_connected = True
#         self._communicator.simulated_robot = self.simulated_robot
#
#         if self.simulated_robot and (self.port or self.address):
#             print('ALSO connected to simulated robot:\n  {}.'.format(
#                 self.simulated_robot))
#
#         if self.tk_root:
    ...
    def _send_command(self, command, data=None, source=None):
        ...
        try:
            ...
            if not self.connector.is_connected:
                RobotError_NoConnection(source).print_message()
            elif self.abort_if_failure:
                raise  # TODO make this really abort instead of raise
            else:
                raise
        except:
            is_failure = True
...
class RobotError_NoConnection(RobotError):
